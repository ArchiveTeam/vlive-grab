  ...
  if allowed(url)
    ...
    if item_type == "video" then
      if string.match(url, "/video/[0-9]+$") then
        ...
        check(
          "https://www.vlive.tv/globalv-web/vam-web/video/v1.0/video/" .. item_value .. "/recommend"
          .. app_id_gcc_locale
        )
        check(
          "https://www.vlive.tv/globalv-web/vam-web/env/v1.0/languages"
          .. app_id_gcc_locale
        )
        check(
          "https://www.vlive.tv/globalv-web/vam-web/member/v1.0/channel-" .. channel_code .. "/me"
          .. "?appId=" .. app_id
          .. "&fields=" .. fields_me
          .. gcc_locale
        )
        check(
          "https://www.vlive.tv/globalv-web/vam-web/board/v1.0/channel-" .. channel_code .. "/groupedBoards"
          .. "?appId=" .. app_id
          .. "&fields=" .. fields_grouped_boards
          .. "&filter="
          .. gcc_locale
        )
        check(
          "https://www.vlive.tv/globalv-web/vam-web/comment/v1.0/post-" .. post_id .. "/starComments"
          .. "?appId=" .. app_id
        ...
        check(
          "https://www.vlive.tv/globalv-web/vam-web/vhs/store/v1.0/channels/" .. channel_code
          .. "?appId=" .. app_id
          .. "&platformType=PC"
          .. gcc_locale
        )
      end
      if string.match(url, "/inkey%?") then
        local json = JSON:decode(html)
        local inkey = json["inkey"]
        check(
          "https://apis.naver.com/rmcnmv/rmcnmv/vod/play/v2.0/" .. vod_id
          .. "?key=" .. inkey
          .. "&pid=rmcPlayer_16692457559726800" -- partially unix time and partially random
          .. "&sid=2024"
          .. "&ver=2.0"
          .. "&devt=html5_pc"
          .. "&doct=json"
      ...
      if string.match(url, "/vod/play/v2%.0/") then
        ...
        for _, stream_data in pairs(json["streams"]) do
          ...
          local max_size = nil
          local max_data = nil
          for _, video_data in pairs(stream_data["videos"]) do
            local bit_rate = video_data["bitrate"]["video"]
            if not max_size or bit_rate > max_size then
              max_size = bit_rate
              max_data = video_data
            end
          end
        ...
        for _, video_data in pairs(json["videos"]["list"]) do
          ...
          if not max_size or bitrate > max_size then
            max_size = bitrate
...
wget.callbacks.finish = function(start_time, end_time, wall_time, numurls, total_downloaded_bytes, total_download_time)
  ...
  for key, data in pairs({
    ["vlive-c9mredaro5tj7ka"] = discovered_items,
    ["urls-2iw8ppt6d67yvkl"] = discovered_outlinks
  }) do
